...
[("2021.09", 1, "SUCCESS"), ("2021.09", 3, "FAILED"), ("2021.07", 4, "PENDING")])
...
@pytest.mark.parametrize("release_name, step_num, status", \
    [("invalid_release", 1, "SUCCESS"), ("2021.09", -1, "SUCCESS"), ("invalid release", -1, "SUCCESS"), ("2021.09", 1, "BADSTATUS")])
def test_put_task_status_failing(release_name, step_num, status):
    reseed()
    put_response = client.put(f"/thor-admin/releases/{release_name}/tasks/{step_num}", json={"status": status}, headers={"Content-Type": "application/json"})
    ...
    if status == "SUCCESS":
        ...
        assert put_response.json() == {
            "detail": [{
                "loc":["body","status"],
                "msg":f"\"{status}\" is not a valid status. "
            }]
        }